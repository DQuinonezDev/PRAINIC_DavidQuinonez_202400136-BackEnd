...
class CursoAprobado {
  static async asignar({ id_usuario, id_curso }) {
  ...
  static async obtenerPorUsuario(id_usuario) {
    ...
    const [rows] = await pool.query(
      `SELECT ca.id_curso, c.nombre, c.seccion, 
  ...
  static async eliminar({ id_usuario, id_curso }) {
    const pool = await dbmysql();
    const [result] = await pool.query(
      `DELETE FROM cursos_aprobados WHERE id_usuario = ? AND id_curso = ?`,
      [id_usuario, id_curso]
    );
